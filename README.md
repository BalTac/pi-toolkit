...
| **`deepseek-rates`** | Shows the **selected model's in/out/cache rates** per 1M tokens (for the period in effect) plus a live **peak/off-peak** badge in pi's status bar. Auto-activates when the current provider is DeepSeek. v3.0 fetches the official rates + peak schedule from the DeepSeek pricing page (DeepSeek exposes no pricing API) and cross-checks the live model list via `GET /models`: the peak/off-peak badge is re-evaluated every 5 min (local clock math, no network) while prices are re-fetched only when the cache is older than a day, cached to `~/.pi/deepseek-rates-cache.json`; falls back to the model registry when offline. v2.0 dropped session cost and remaining credit — other tools display those (pi-usage, pi-agent-budget). | Yes — network on refresh (docs page + optional `/models` with the stored key); cache keeps it working offline |
...
| `deepseek-rates` | ✅ | Network on refresh: parses the official DeepSeek pricing page for rates + peak schedule and cross-checks `GET /models`; caches to `~/.pi/deepseek-rates-cache.json` and falls back to the model registry offline. |
...
## Local patches (third-party tweaks)

Some behaviours need a tweak to a package we do not own. Those live in
`scripts/` as **idempotent, revertible** scripts — never as manual edits:

```bash
node scripts/patch-llm-wiki-status.mjs --check    # report state
node scripts/patch-llm-wiki-status.mjs            # apply (idempotent)
node scripts/patch-llm-wiki-status.mjs --revert   # rollback
```
